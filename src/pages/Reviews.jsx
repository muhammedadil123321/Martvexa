...
export default function Reviews() {
  ...
  return (
    <div className="bg-[linear-gradient(160deg,#F3E9D2_0%,#F8F2E4_45%,#FBF9F4_100%)] text-[#221C18] py-12 md:py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <LocalStyles />
      <div className="max-w-6xl mx-auto">
        ...
        {/* ================= REVIEWS CONTENT / SKELETON LOADING ================= */}
        {loading ? (
          /* SKELETON LOADING CARDS */
          <div className="flex overflow-x-auto no-scrollbar gap-5 py-3">
            {[...Array(3)].map((_, index) => (
              <div
                key={index}
                className="shrink-0 w-[85%] sm:w-[calc(50%-10px)] lg:w-[calc(33.333%-13.33px)] flex flex-col"
              >
                <div className="h-full bg-white/85 backdrop-blur-md border border-[#B57A25]/20 rounded-2xl p-6 shadow-sm flex flex-col justify-between animate-pulse min-h-[200px]">
                  <div>
                    {/* Stars Skeleton */}
                    <div className="w-24 h-4 bg-gray-200 rounded-md mb-4" />
                    {/* Comment Lines Skeleton */}
                    <div className="w-full h-3 bg-gray-200 rounded-md mb-2" />
                    <div className="w-4/5 h-3 bg-gray-200 rounded-md mb-2" />
          ...
          >
            {reviews.map((rev) => {
              ...
              const displayName = rev.userName || "Customer";
              ...
              return (
                ...
                >
                  <div className="review-card h-full bg-white/85 backdrop-blur-md border border-[#B57A25]/20 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    ...
                    <div>
                      {/* 1. STARS RATING */}
                      <div className="flex items-center gap-1 text-[#B57A25] mb-3" aria-label={`${ratingCount} out of 5 stars`}>
